		...
		this.selectRowImpl = function(){
			...
			$(findEle).click( function( e ) {
					...
					if(pageType == 'gddd'){
						...
						//提交后台查看爬取数据 
						var formObj = "deviceid="+aaa.device_id+"&username="+aaa.username;
						$.ajax({
				    		url:document.getElementById('contextPath').value+'/management/position/getDdCoordinate',
				    		type:'post',
				    		data:formObj,
							cache:false,
							success:function(response){
								if(response == "未作记录"){
									jAlert('未找到记录','提示');
									document.getElementById('hzb').value="";
									document.getElementById('zzb').value="";
								}else{
									var mgs = response.split("%");
									document.getElementById('hzb').value=mgs[0];
									document.getElementById('zzb').value=mgs[1];
								}
							}
				   		 });	
					}
					if(pageType == 'gdgj'){
						...
						//使用ajxa提交到后台查询设备ID的时间记录
						//提交后台查看爬取数据 
						var formObj = "deviceid="+aaa.device_id;
						$.ajax({
				    		url:document.getElementById('contextPath').value+'/management/position/getGjCoordinate',
				    		type:'post',
				    		data:formObj,
							cache:false,
							success:function(response){
								if(response == "未作记录"){
									...
									document.getElementById('zb').value=response;
								}
							}
				   		 });	
					}
				    //end
					gdiframe.location.reload(true);
