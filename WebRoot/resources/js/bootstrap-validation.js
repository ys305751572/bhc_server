...
!function($) {
    ...
    $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'number', validate: function(value) {return (!/^[0-9]\d*$/.test(value));}, defaultMsg: '请输入整数。'},
			{name: 'double', validate: function(value) {return (!/^-?\d+\.?\d*$/.test(value));}, defaultMsg: '请输入数字。'},
	...
	var validationForm1 = function(obj) { // 表单验证方法
		var _objform = $(obj);
        $(obj).submit(function() { // 提交时验证
            if (formState) { // 重复提交则返回
                return false;
		...
		jConfirm("是否确认提交？","提示",function(r){
            if(r){
			document.getElementById(_objform.attr('id')).submit();
             }
			 else{
				 formState = false; 
			 }
