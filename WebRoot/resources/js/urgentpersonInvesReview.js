	...
	tools.dataTable = function(options) {
		...
		this.init = function() {
			...
			this.oTable = $('.datatable').dataTable({
				...
				"sPaginationType" : "two_button", // 翻页界面类型
		...
		this.insertHTML = function(){
			...
			var htmlStr = '<div class="row-fluid sortable">';
				...
				for (var  i= 0; i < this.columns.length; i++) {
					...
					var awidthStr = this.columns[i].style?'style="'+this.columns[i].style+'"':'';
					htmlStr+= '<th '+awidthStr+widthStr+'>'+this.columns[i].text+'</th>';
				}
				htmlStr+= '						</tr>';
				htmlStr+= '					</thead>';
				htmlStr+= '				 <tbody>';
		...
		this.selectRowImpl = function(){
			t.selectedRow = t.store[0];
			if(!options.render)//没有渲染目标,则不实现拖动效果
				return;		
			var findEle = '#'+options.render+' table:eq(1) tr';
			$(findEle).click( function( e ) {
					var rowIndex = $(this).context.rowIndex-1;
					var aaa =t.store[rowIndex];
					//start
					var pageType = document.getElementById('pageType').value;//因为2个页面查询调用的同一个JS，所以设定pagetype用来区分
						//处理定点页面的方法
						//使用ajxa提交到后台查询设备ID的时间记录
						//提交后台查看爬取数据 
						var formObj = "userid="+aaa.userId;
						//判断pageType 来确定集合类型
						if(pageType=="userurgent"){
							$.ajax({
					    		url:document.getElementById('contextPath').value+'/management/urgentperson/getuserurgentperson',
					    		type:'post',
					    		data:formObj,
								cache:false,
								success:function(response){
									document.getElementById('userid').value=aaa.userId;
									if(response==""){
										document.getElementById('urgentxm').value="";
										document.getElementById('telephone').value="";
										document.getElementById('address').value="";
										document.getElementById('urgentperson_id').value="";
										
									}else{
										var msg = response;
										var msg = eval('(' + response + ')');
										document.getElementById('urgentxm').value=msg.urgentxm;
										document.getElementById('telephone').value=msg.telephone;
										document.getElementById('address').value=msg.address;
										document.getElementById('urgentperson_id').value=msg.urgentperson_id;
										if(msg.sffs=='1'){
											document.getElementById('sffs').checked = true;
										}else{
											document.getElementById('sffs').checked = false;
										}
						...
						if(pageType=="guardian"){
							$.ajax({
					    		url:document.getElementById('contextPath').value+'/management/urgentperson/getguardianperson',
					    		type:'post',
					    		data:formObj,
								cache:false,
								success:function(response){
									document.getElementById('userid').value=aaa.userId;
									if(response==""){
										document.getElementById('urgentxm1').value="";
										document.getElementById('telephone1').value="";
										document.getElementById('address1').value="";
										document.getElementById('gx1').value="";
										document.getElementById('urgentperson_id1').value="";
										
										document.getElementById('urgentxm2').value="";
										document.getElementById('telephone2').value="";
										document.getElementById('address2').value="";
										document.getElementById('gx2').value="";
										document.getElementById('urgentperson_id2').value="";
										
										document.getElementById('urgentxm3').value="";
										document.getElementById('telephone3').value="";
										document.getElementById('address3').value="";
										document.getElementById('gx3').value="";
										document.getElementById('urgentperson_id3').value="";
										
									}else{
										var msg = response;
										var msg = eval('(' + response + ')');
										document.getElementById('urgentxm1').value=msg.urgentxm1;
										document.getElementById('telephone1').value=msg.telephone1;
										document.getElementById('address1').value=msg.address1;
										document.getElementById('gx1').value=msg.gx1;;
										document.getElementById('urgentperson_id1').value=msg.urgentperson_id1;
										
										document.getElementById('urgentxm2').value=msg.urgentxm2;
										document.getElementById('telephone2').value=msg.telephone2;
										document.getElementById('address2').value=msg.address2;
										document.getElementById('gx2').value=msg.gx2;;
										document.getElementById('urgentperson_id2').value=msg.urgentperson_id2;
										
										document.getElementById('urgentxm3').value=msg.urgentxm3;
										document.getElementById('telephone3').value=msg.telephone3;
										document.getElementById('address3').value=msg.address3;
										document.getElementById('gx3').value=msg.gx3;
										document.getElementById('urgentperson_id3').value=msg.urgentperson_id3;
									}
								}
					   		 });	
						}
