	...
	tools.dataTable = function(options) {
		...
		this.selectRowImpl = function(){
			...
			$(findEle).click( function( e ) {
					...
					var pageType = document.getElementById('pageType').value;//因为2个页面查询调用的同一个JS，所以设定pagetype用来区分
						...
						//提交后台查看爬取数据 
						var formObj = "deviceid="+aaa.device_id+"&userid="+aaa.userId;
						//判断pageType 来确定查询测量类型
						...
						$.ajax({
				    		...
				    		type:'post',
				    		data:formObj+"&measureType="+measureType,
							cache:false,
							success:function(response){
								if(response == "未找到记录"){
									...
									document.getElementById('strxml').value=response;
		...
		this.fnRowCallback = function(nRow, aData, iDisplayIndex, listIndex) {
			...
			for (var key in aData) {//截断长度过长的字符串
				...
				nRow.childNodes[4].hidden= true;
				nRow.childNodes[5].hidden= true;
