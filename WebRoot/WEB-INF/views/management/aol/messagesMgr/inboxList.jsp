    ...
    return format;
		...
		<script type="text/javascript">
				...
				var columns = [ {'text':'<div align="center"><input type="checkbox" id="ckb_selectAll" name="ckb_selectAll" title="选中/取消选中"></div>','render':ckbRender,'width':'10px'},
								...
								{'text':'发件人','dataIndex':'messagesSenderName','width':'200px','render':messagesSenderNameRender},
				...
				$(function() {
					dataTableObj  = new czTools.dataTable({"columns":columns,"render":"inboxListDataTable",
												"url":"${contextPath}/management/messages/getinboxDataList",
												"para":arrayObj,
				...
				//收件人
				function messagesSenderNameRender(row){
					if(row.messagesState == 0){
						return '<div class="mesStateCt">' + row.messagesSenderName + '</div>';
					} else {
						return '<div>' + row.messagesSenderName + '</div>';
					}
				}
				//标题
				...
				function messagesTimeRender(row){
					...
					if(row.messagesState == 0){
						return '<div class="mesStateCt">' + messagesTime + '</div>';
					} else {
						return '<div>' + messagesTime + '</div>';
					}
				}
				...
				function markReadMsg(){
					...
					if(valArr.length <= 0){
						jAlert("请选择需标记的消息记录",'提示');
						return;
					}
			        var vals = valArr.join(',');
			        $.post("${contextPath}/management/messages/markReadMsg",{"msgId":vals},function(result){
						if(result.success){
							searchBtnClick();
						}
						jAlert(result.msg,'提示');
		...
		<div class="row-fluid z-ulnone" id="proList">
			<div class="box span12">			
				...
				<div class="breadcrumb">
					...
					<li style="color: #c5c5c5">|</li>
					<li><a href="javascript:markReadMsg();" class="button button-rounded button-flat button-tiny" style="width: 100px;"><i class="icon-17" style="width: 20px; height: 20px; line-height: 20px;"></i>&nbsp;标记为已读</a></li>
				</div>
				<!-- 操作按钮end -->
