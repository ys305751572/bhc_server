...
function radioChange(_type){
	var treeUrl = "";
	if(_type == 'ALL' || _type == 'USER'){
		treeUrl = "${contextPath}/management/messages/getOrgAndUserTreeData";
	} else if(_type == 'AGENT'){
		treeUrl = "${contextPath}/management/messages/getOrgTreeData";
	}

	$('#messagesSendee').combotree('reload', treeUrl);
}
	...
	<div class="row-fluid">
		<div id="content" class="span12">
			<div class="row-fluid z-ulnone">
				<form class="form-horizontal" method="post" id="messageForm" name="messageForm" action="${contextPath}/management/messages/saveMessage" onsubmit="return check_task()" enctype="multipart/form-data">
					<!--box span12 start-->
					<div class="box span12">
						<div class="box-header well z-h2">
							<div class="controls" style="margin-left: 10px;">
								<button id="btnSendTop" name="btnSendTop" type="button" class="btn btn-primary" onclick="submitMessageFrom('send');">发送</button>
								<button id="btnSaveTop" name="btnSaveTop" type="button" class="btn btn-primary" onclick="submitMessageFrom('save');">存草稿</button>
							</div>
						</div>
									...
									<tr>
										<td>
											<div class="control-group" style="margin-top: 10px;">
											  <label class="control-label" style="width:60px;">收件方</label>
											  <div class="controls" style="margin-left: 80px;">
											  	  <label class="checkbox inline">
											        <input type="radio" name="sendeeType" id="sendeeType1" value="1" onchange="radioChange('USER');" <c:if test="${empty messages}">checked="checked"</c:if> <c:if test="${messages.sendeeType == 1}">checked="checked"</c:if> > 用户
												  </label>
												  <label class="checkbox inline">
													<input type="radio" name="sendeeType" id="sendeeType2" value="2" onchange="radioChange('AGENT');" <c:if test="${messages.sendeeType == 2}">checked="checked"</c:if> > 代理商
												  </label>
												  <label class="checkbox inline">
													<input type="radio" name="sendeeType" id="sendeeType3" value="3" onchange="radioChange('ALL');" <c:if test="${messages.sendeeType == 3}">checked="checked"</c:if> > 用户和代理商
												  </label>
											  </div>
											</div>
										</td>
										...
										<td>
											<div class="control-group">
												<div class="controls" style="margin-left: 20px;">
													...
													<button id="btnSaveBottom" name="btnSaveBottom" type="button" class="btn btn-primary" onclick="submitMessageFrom('save');">存草稿</button>
												</div>
											</div>
										</td>
									</tr>
