...
<html lang="en">
	<head>
		...
		<script type="text/javascript">
				$(function() {
					$('#startingSide').tree({ 
						url:"${contextPath}/management/devices/getOrgAndDevTreeData",
						checkbox: true,
					...
					$('#endingSide').tree({ 
						url:"${contextPath}/management/devices/getOrgTreeData",
						onClick: function(node){
				...
				function searchBtnClick(){
					var search_deviceSerial = $("#deviceSerial").val();
					$("#startingSide").tree("search", search_deviceSerial);
				}
				...
				function doChangeDev(){
					...
					for(var i=0;i<checkNodes.length;i++){
						...
						if(nodeid.substr(0, 2) == "D_" || nodeid.substr(0, 2) == "W_"){
							devIds = devIds + nodeid + "@@";
					...
					var isOkMsg = "是否确认转移所选设备至接收单位？";
					jConfirm(isOkMsg,"提示",function(r){
						if(r) { 
							$.post("${contextPath}/management/devices/doChangeDev",{"devIds":devIds, "orgId":orgId},function(result){
								if(result.success){
									$("#orgId").val("");
		...
		<div class="row-fluid z-ulnone" id="proList">
			<div class="box span12">			
				...
				<div class="breadcrumb">
					<li><a href="javascript:doChangeDev();" class="button button-rounded button-flat button-tiny" style="width: 60px;"><i class="icon-8" style="width: 20px; height: 20px; line-height: 20px;"></i>&nbsp;转移</a></li>
				</div>
				<!-- 操作按钮end -->
				...
				<div class="box-content" style="margin: -15px;">
				  <div style="float: left; width: 90%; margin: 0px 15px 10px;">
				  	<div style="height: 28px; line-height: 28px;">
						<form id="form1" name="form1" class="form-horizontal" style="margin: 0px 0px;" action="" method="post" enctype="multipart/form-data">
							<span style="font-size: 15px;font-weight: normal;line-height: 18px;">设备序列号：</span>
							<input id="deviceSerial" name="deviceSerial" type="text" value="" style="width: 200px; height: 15px;" />
				  ...
				  <div style="float: left; width: 30%; margin: 0px 15px 9px;">
					<div style="float: left; width: 100%; margin-bottom: 5px;">
						<span style="font-size: 15px;font-weight: normal;line-height: 18px;">请选择设备：</span>
					</div>
